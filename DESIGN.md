...
Natural preference is revealed comparatively across contexts, not by any single run or context. The context that produces the most stable archetype at routine, middling stress is the strongest indicator of natural orientation — not the most relaxed context. Both extra stress and extra relaxation are deviations: extra stress triggers the stress response (reaching toward Receiver behavior), extra relaxation triggers the ease response (drifting toward Supplier behavior). A context that is unusually demanding or unusually comfortable will suppress the natural archetype in favor of a neighbor. Variance across runs within a context is therefore a signal of stress-level fluctuation, not archetype instability.
